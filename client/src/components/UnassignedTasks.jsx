...
export function UnassignedTasks({ onBack }) {
  ...
  const fetchUnassignedTasks = async () => {
    try {
      ...
      // Fetch both projects and unassigned tasks
      const [fetchedProjects, unassignedTasks] = await Promise.all([
        projectsApi.getAll(),
        tasksApi.getAll({ assignedTo: null })
      ]);
      ...
      const tasksWithSubtasks = await Promise.all(
        unassignedTasks.map(async (task) => {
          try {
            ...
            return {
              ...task,
              subtasks: subtasks.filter(st => !st.answered)
            };
          } catch (err) {
      ...
      if (isMobile && tasksWithSubtasks.length > 0) {
        const firstTaskProjectId = tasksWithSubtasks[0].project_id;
  ...
  if (loading) {
    return (
      <div style={styles.container}>
        <div style={styles.loading}>Loading unassigned tasks...</div>
      </div>
    );
  }
      ...
      <div style={styles.container}>
        ...
        {tasksByProject.length === 0 ? (
          <div style={styles.empty}>
            ...
            <p style={styles.emptyText}>All tasks are assigned!</p>
            <p style={styles.emptySubtext}>No unassigned tasks found</p>
          </div>
        ) : (
          <>
            {/* Project Tabs */}
