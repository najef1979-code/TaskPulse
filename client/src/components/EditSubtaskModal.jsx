...
export function EditSubtaskModal({ subtask, onClose, onUpdate, onTasksRefresh }) {
  ...
  const [type, setType] = useState(subtask.type);
  const [providedFile, setProvidedFile] = useState(subtask.provided_file);
  const [fileReference, setFileReference] = useState(subtask.file_reference || '');
  const [assignedTo, setAssignedTo] = useState(subtask.assigned_to);
  ...
  const handleUpdate = async () => {
    ...
    if (providedFile !== 'no_file') {
      ...
      updates.file_reference = fileReference.trim();
    }
    ...
    <div style={styles.overlay} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div style={styles.header}>
          <h2 style={styles.title}>Edit Subtask</h2>
